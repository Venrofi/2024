...
describe("promises", () => {
	...
	describe("race", () => {
		...
		test("resolves with value of the fastest successful promise", () => {
			const FAST = "FAST";
			return expect(
				race([slowestPromise("SLOWEST"), slowerPromise("SLOWER"), fastPromise(FAST)])
		...
		test("rejects if the fastest promise fails even if other slower promises succeed", () => {
			return expect(race([slowestPromise("SLOWEST"), failedPromise(null)])).rejects.toEqual(
				failedCallback
			);
		});
	});
	...
	describe("any", () => {
		...
		test("resolves with value of fastest successful promise", () => {
			const FAST = "FAST";
			return expect(
				any([slowestPromise("SLOWEST"), slowerPromise("SLOWER"), fastPromise(FAST)])
			).resolves.toEqual(FAST);
		});
		...
		test("resolves with value of fastest successful promise even if faster promises fail", () => {
			...
			return expect(any([failedPromise(null), slowestPromise(SLOWEST)])).resolves.toEqual(
				SLOWEST
			);
		});

		test("rejects with array of errors if all promises fail", () => {
			return expect(any([failedPromise(null), failedPromise(null)])).rejects.toEqual([
				failedCallback,
				failedCallback,
			]);
		});
